...
# @pytest.mark.asyncio
# async def test_process_endpoint_missing_file_url(api_config, test_logger):
#     """Test POST request with missing file_url parameter.
#     
#     Validates:
#     - HTTP status code is 422 (Unprocessable Entity)
#     - Error details indicate missing required parameter
#     """
#     async with async_playwright() as p:
#         context = await p.request.new_context(
#             base_url=api_config["base_url"],
...
#         )
#         # Prepare query parameters WITHOUT file_url
#         query_params = {
#             "file_type": api_config["file_type"],
#             "file_id": api_config["file_id"],
#             "builder_id": api_config["builder_id"],
#             "entity_id": api_config["entity_id"],
#             # file_url is intentionally omitted
#         }
#         
#         test_logger.log_request("POST", api_config["endpoint"], params=query_params)
...
#         )
#         
#         response_data = await response.json() if response.headers.get("content-type") == "application/json" else {}
#         test_logger.log_response(response.status, response_data)
#         
#         # Assertion: expect 422 (Unprocessable Entity)
...
#         test_logger.log("✓ Missing file_url correctly returns 422 status")


# @pytest.mark.asyncio
# async def test_process_endpoint_invalid_file_type(api_config, test_logger):
#     """Test POST request with invalid file_type parameter.
#     
#     Validates:
#     - HTTP status code is 422 (Unprocessable Entity)
#     - Error details indicate invalid file_type
#     """
#     async with async_playwright() as p:
#         context = await p.request.new_context(
#             base_url=api_config["base_url"],
#             timeout=api_config["timeout"],
#         )
#         # Prepare query parameters with INVALID file_type
#         query_params = {
#             "file_url": api_config["file_url"],
...
#         assert isinstance(response_data.get("detail"), list), "Error details should be a list"
#         test_logger.log("✓ Invalid file_type correctly returns 422 status")


# @pytest.mark.asyncio
# async def test_process_endpoint_missing_builder_id(api_config, test_logger):
#     """Test POST request with missing builder_id parameter.
#     
#     Note: builder_id appears to be optional based on API behavior.
...
#             timeout=api_config["timeout"],
#         )
#         # Prepare query parameters WITHOUT builder_id
#         query_params = {
#             "file_url": api_config["file_url"],
#             "file_type": api_config["file_type"],
#             "file_id": api_config["file_id"],
#             # builder_id is intentionally omitted
#             "entity_id": api_config["entity_id"],
#         }
#         
#         test_logger.log_request("POST", api_config["endpoint"], params=query_params)
#         test_logger.log("Testing: missing builder_id")
#         
#         # Send request
#         response = await context.post(
#             api_config["endpoint"],
...
#         test_logger.log_response(response.status, response_data)
#         
#         # Note: API accepts missing builder_id and returns 200
#         assert response.status == 200, f"Expected 200, got {response.status}"
#         assert response_data.get("status") == "SUCCESS", "Expected SUCCESS status"
#         test_logger.log("✓ API accepts request without builder_id parameter")
